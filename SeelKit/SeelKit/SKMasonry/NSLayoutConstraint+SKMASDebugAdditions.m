//
//  NSLayoutConstraint+SKMASDebugAdditions.m
//  SKMASonry
//
...
#import "NSLayoutConstraint+SKMASDebugAdditions.h"
#import "SKMASConstraint.h"
#import "SKMASLayoutConstraint.h"

@implementation NSLayoutConstraint (SKMASDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
            @(SKMASLayoutPriorityDefaultHigh)      : @"high",
            @(SKMASLayoutPriorityDefaultLow)       : @"low",
            @(SKMASLayoutPriorityDefaultMedium)    : @"medium",
            @(SKMASLayoutPriorityRequired)         : @"required",
            @(SKMASLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(SKMASLayoutPriorityDefaultHigh)                 : @"high",
            @(SKMASLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(SKMASLayoutPriorityDefaultMedium)               : @"medium",
            @(SKMASLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(SKMASLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(SKMASLayoutPriorityDefaultLow)                  : @"low",
            @(SKMASLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(SKMASLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(SKMAS_key)] && [obj SKMAS_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj SKMAS_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != SKMASLayoutPriorityRequired) {
